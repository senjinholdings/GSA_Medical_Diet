...
class RankingApp {
    ...
    // モバイル用：クリニックスライダー生成（<=768px表示）
    updateClinicSlider(clinics, ranking) {
        try {
            const slider = document.getElementById('clinic-slider');
            const slides = document.getElementById('clinic-slides');
            ...
            let ordered = [];
            if (ranking && ranking.ranks) {
                const sorted = Object.entries(ranking.ranks)
                    .sort((a,b) => parseInt(a[0].replace('no','')) - parseInt(b[0].replace('no','')))
                    .map(([_, id]) => id);
                ordered = sorted
                    .map(id => clinics.find(c => c.id == id))
                    .filter(Boolean);
            }
            if (ordered.length === 0) ordered = clinics.slice(0, 10);

            const regionId = this.currentRegionId || '013';

            ordered.forEach((clinic, idx) => {
                const clinicCode = clinic.code;
                const getText = (key, def='') => this.dataManager.getClinicText(clinicCode, key, def);
                let logoPath = getText('クリニックロゴ画像パス', '');
                if (!logoPath) logoPath = `../common_data/images/clinics/${clinicCode}/${clinicCode}-logo.webp`;
                const evalScore = getText('総合評価', '4.5');
                const field2 = 'comparison2';
                const field3 = 'comparison3';
                const val2 = this.dataManager.processDecoTags(getText(field2, ''));
                const val3 = this.dataManager.processDecoTags(getText(field3, ''));
                const rankNum = (function(){
                    if (!ranking || !ranking.ranks) return idx+1;
                    const entry = Object.entries(ranking.ranks).find(([k,v]) => v == clinic.id);
                    if (!entry) return idx+1;
                    return parseInt(entry[0].replace('no','')) || (idx+1);
                })();

                const slide = document.createElement('div');
                slide.className = 'clinic-slide';
                slide.innerHTML = `
                    <div class="slide-header">
                        <img src="${logoPath}" alt="${clinic.name}">
                        <div class="slide-title">${clinic.name}</div>
                    </div>
                    <div class="slide-body">
                        <div>総合評価: <span class="ranking_evaluation">${evalScore}</span> <span class="star5_rating" data-rate="${evalScore}"></span></div>
                        <div>${val2}</div>
                        <div>${val3}</div>
                    </div>
                    <div class="slide-actions">
                        <a class="link_btn" href="${this.urlHandler.getClinicUrlWithRegionId(clinic.id, rankNum)}" target="_blank" rel="noopener">公式サイト ></a>
            ...
            // コントロール
            const prev = document.getElementById('clinic-prev');
            const next = document.getElementById('clinic-next');
            const scrollByPage = (dir) => {
                const cont = slider.querySelector('#clinic-slides');
                const w = cont.clientWidth;
                cont.scrollBy({ left: dir * w, behavior: 'smooth' });
            };
            if (prev && next) {
            ...
            console.warn('updateClinicSlider error:', e);
        }
    }
    ...
    updatePageContent(regionId) {
        try {
            ...
            // 比較表の更新（タブ撤廃＋縦=項目/横=クリニック）
            this.updateComparisonTable(allClinics, ranking);
            
            // モバイル用クリニックスライダーの更新
            this.updateClinicSlider(allClinics, ranking);
    ...
    // 比較表の更新（行×列を入れ替えた横=クリニック、縦=項目表示）
    updateComparisonTable(clinics, ranking) {
        ...
        // 縦横入れ替え版の比較表を生成
        this.generateTransposedComparisonTable(rankedClinics);
    ...
    // 行×列を入れ替えた比較表を生成
    generateTransposedComparisonTable(rankedClinics) {
        const headerRow = document.getElementById('comparison-header-row');
        const tbody = document.getElementById('comparison-tbody');
        ...
        rankedClinics.forEach((clinic, index) => {
            ...
            const logoPath = logoPathFromCsv || `../common_data/images/clinics/${clinicCode}/${clinicCode}-logo.webp`;

            const rankNum = clinic.rank || index + 1;
            const redirectUrl = `./redirect.html#clinic_id=${clinic.id}&rank=${rankNum}&region_id=${regionId}`;
            const onclick = `onclick=\"localStorage.setItem('redirectParams', JSON.stringify({clinic_id: '${clinic.id}', rank: '${rankNum}', region_id: '${regionId}'})); setTimeout(() => { window.open('${redirectUrl}', '_blank'); }, 10); return false;\"`;

            th.innerHTML = `
                <img src="${logoPath}" alt="${clinic.name}" width="80"><br>
                <a href="#" ${onclick} class="clinic-link" style="cursor: pointer;">${clinic.name}</a>
            `;
            headerRow.appendChild(th);
        });
        ...
        const headerConfig = this.dataManager.clinicTexts['比較表ヘッダー設定'] || {};
        const rows = [
            { key: 'comparison1', label: headerConfig['比較表ヘッダー1'] || '総合評価' },
            { key: 'comparison2', label: headerConfig['比較表ヘッダー2'] || '費用' },
            { key: 'comparison3', label: headerConfig['比較表ヘッダー3'] || '特徴' },
            { key: 'comparison4', label: headerConfig['比較表ヘッダー4'] || '矯正範囲' },
            { key: 'comparison5', label: headerConfig['比較表ヘッダー5'] || '目安期間' },
            { key: 'comparison6', label: headerConfig['比較表ヘッダー6'] || '通院頻度' },
            { key: 'comparison7', label: headerConfig['比較表ヘッダー7'] || '実績/症例数' },
            { key: 'comparison8', label: headerConfig['比較表ヘッダー8'] || 'ワイヤー矯正の紹介' },
            { key: 'comparison9', label: headerConfig['比較表ヘッダー9'] || 'サポート' },
            { key: 'official',    label: headerConfig['比較表ヘッダー11'] || '公式サイト' }
        ];
        ...
        // 詳細リンクのスクロール設定
        this.setupDetailScrollLinks();
        // 連続スクロール（ドラッグ/ホイール）を有効化
        this.initializeComparisonSlider(rankedClinics.length);
    }
    ...
    updateComparisonColumnVisibility(start, totalClinics) {
        const visible = this.comparisonVisibleCols || 3;
        const end = Math.min(start + visible - 1, totalClinics - 1);

        const prevStart = this._currentStart ?? 0;
        const prevEnd = Math.min(prevStart + visible - 1, (this.comparisonTotalClinics || totalClinics) - 1);

        const newSet = new Set();
        for (let i = start; i <= end; i++) newSet.add(i);
        const oldSet = new Set();
        for (let i = prevStart; i <= prevEnd; i++) oldSet.add(i);
        ...
        toHide.forEach(i => {
            document.querySelectorAll(`.comparison-table .col-${i}`).forEach(el => {
                ...
                requestAnimationFrame(() => {
                    ...
                    setTimeout(() => {
                        el.classList.remove('anim-exit', 'anim-active');
                        el.classList.add('col-hidden');
                        // 後処理でtransform/opacityをクリア
                        el.style.transform = '';
                        el.style.opacity = '';
                    }, 230);
                });
        ...
        toShow.forEach(i => {
            document.querySelectorAll(`.comparison-table .col-${i}`).forEach(el => {
                ...
                el.classList.add('anim-active');
                setTimeout(() => {
                    el.classList.remove('anim-enter', 'anim-active');
                    el.style.transform = '';
                    el.style.opacity = '';
                }, 230);
            });
