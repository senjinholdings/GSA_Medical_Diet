...
class RankingApp {
    ...
    generateComparisonTable(clinics) {
        ...
        // ヘッダー（項目名）を clinic-texts.json の13〜20番目のキーから動的生成
        const determineDynamicKeys = () => {
            // 有効なクリニックオブジェクトを取得（空キーや空データを除外）
            const clinicEntries = Object.entries(this.dataManager.clinicTexts || {})
                .filter(([name, obj]) => name && typeof obj === 'object' && obj && (obj['クリニック名'] ?? '') !== '');
            if (clinicEntries.length === 0) return [];
            const firstClinicObj = clinicEntries[0][1];
            // オブジェクトのキー順は挿入順（JSONの記述順）を維持する前提
            const allKeys = Object.keys(firstClinicObj);
            // 13〜20番目（1始まり）→ 0始まりでは 12..19
            const startIndex = 12;
            const endIndexExclusive = 20;
            const slice = allKeys.slice(startIndex, endIndexExclusive);
            return slice;
        };

        const dynamicKeys = determineDynamicKeys();

        // thead を動的に更新
        try {
            const theadRow = document.querySelector('.comparison-table table thead tr');
            if (theadRow && Array.isArray(dynamicKeys) && dynamicKeys.length > 0) {
                theadRow.innerHTML = [
                    '<th>クリニック</th>',
                    ...dynamicKeys.map(key => `<th>${key}</th>`),
                    '<th>公式サイト</th>'
                ].join('');
            }
        } catch (e) {
        ...
        clinics.forEach((clinic, index) => {
            ...
            // 動的カラム（13〜20）の値を生成
            const valueCellsHtml = (dynamicKeys || []).map(key => {
                const value = clinicCode ? this.dataManager.getClinicText(clinicCode, key, '') : '';
                // HTMLタグ（<br> など）を許容
                const rendered = this.dataManager && typeof this.dataManager.processDecoTags === 'function'
                    ? this.dataManager.processDecoTags(value || '')
                    : (value || '');
                return `<td class="">${rendered}</td>`;
            }).join('');
