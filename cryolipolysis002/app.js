...
class RankingApp {
    constructor() {
        ...
        this.comparisonScrollContainer = null;
    ...
    prepareComparisonDom() {
        ...
        if (layout === 'A') {
            ...
            if (this.preparedComparisonLayout === 'A') {
                this.updateComparisonStickyOffsets();
                return;
            }
    ...
    updateComparisonStickyOffsets() {
        const offset = this.getComparisonStickyOffset();
        const matrix = this.getComparisonMatrix();
        if (matrix) {
            matrix.style.setProperty('--comparison-sticky-top', `${offset}px`);
        } else {
            const fallbackMatrix = document.querySelector('.comparison-table--matrix');
            if (fallbackMatrix) {
                fallbackMatrix.style.removeProperty('--comparison-sticky-top');
            }
        }
        if (this.comparisonStickyHeader) {
            this.comparisonStickyHeader.style.setProperty('--comparison-sticky-offset', `${offset}px`);
        }
    }

    ensureComparisonStickyHeader(matrix) {
        if (!matrix || !this.isComparisonLayoutB()) {
            return;
        }
        ...
        if (!this.comparisonStickyHeader) {
            ...
            if (typeof ResizeObserver !== 'undefined' && headerElement) {
                this.headerResizeObserver = new ResizeObserver(() => this.updateComparisonStickyOffsets());
                this.headerResizeObserver.observe(headerElement);
            }
        }

        const scrollContainer = this.getComparisonScrollContainer();
        if (this.comparisonScrollContainer && this.comparisonScrollContainer !== scrollContainer && this.boundMatrixScrollHandler) {
            this.comparisonScrollContainer.removeEventListener('scroll', this.boundMatrixScrollHandler);
        }
        if (scrollContainer && this.boundMatrixScrollHandler) {
            ...
            this.comparisonScrollContainer = scrollContainer;
        }
    ...
    syncComparisonStickyHorizontalScroll() {
        ...
        if (scrollContainer && this.boundMatrixScrollHandler && this.comparisonScrollContainer !== scrollContainer) {
            scrollContainer.addEventListener('scroll', this.boundMatrixScrollHandler, { passive: true });
        }
        this.comparisonScrollContainer = scrollContainer;
    ...
    teardownComparisonStickyHeader() {
        if (this.comparisonScrollContainer && this.boundMatrixScrollHandler) {
            this.comparisonScrollContainer.removeEventListener('scroll', this.boundMatrixScrollHandler);
        }
        this.comparisonScrollContainer = null;
        ...
        const matrix = document.querySelector('.comparison-table--matrix');
        if (matrix) {
            matrix.style.removeProperty('--comparison-sticky-top');
        }
    }
            ...
            detailItem.innerHTML = `
                ...
                ${(() => {
                    ...
                    const slidesHtml = imagesForClinic.map((img, idx) => {
                        ...
                        const fallbackAttr = img.fallbacks[1]
                            ? ` onerror=\"if (!this.dataset.webpFallback) { this.dataset.webpFallback = '1'; this.src='${img.fallbacks[1]}'; }\"`
                            : '';
                        return `
